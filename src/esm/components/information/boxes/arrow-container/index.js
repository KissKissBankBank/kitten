...
export const ArrowContainer = _ref => {
  let {
    children,
    className,
    color,
  ...
  const internalPadding = typeof padding === 'number' ? pxToRem(padding) : padding;
  const internalBorderRadius = typeof borderRadius === 'number' ? pxToRem(borderRadius) : borderRadius;
  const internalSize = typeof size === 'number' ? pxToRem(size) : size;
  const internalBorderWidth = typeof borderWidth === 'number' ? pxToRem(borderWidth) : borderWidth;

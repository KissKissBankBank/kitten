...
export const Marger = props => {
  ...
  const isPropWithViewportRange = (propName, viewportRange) => props[propName] && props[propName]["from" + upcaseFirst(viewportRange)];

  const viewportRangeCssRule = viewportRange => "@media (min-width: " + ScreenConfig[viewportRange.toUpperCase()].min + "px)";
  ...
  const viewportRanges = Object.keys(ScreenConfig).map(size => size.toLowerCase()).filter(size => size !== 'xxs');
...
const StyledMarger = styled.div.withConfig({

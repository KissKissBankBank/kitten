...
const ArrowContainer = _ref => {
  ...
  const internalPadding = typeof padding === 'number' ? (0, _typography.pxToRem)(padding) : padding;
  const internalBorderRadius = typeof borderRadius === 'number' ? (0, _typography.pxToRem)(borderRadius) : borderRadius;
  const internalSize = typeof size === 'number' ? (0, _typography.pxToRem)(size) : size;
  const internalBorderWidth = typeof borderWidth === 'number' ? (0, _typography.pxToRem)(borderWidth) : borderWidth;
  ...
  const borderDistance = borderWidth % 2 === 0 ? Math.floor(rawDistanceValue) : Math.ceil(rawDistanceValue);

...
Features:
  ...
  `Illustrations`: Add the following illustration components:
    - `DeskIllustration`,
    - `SpeakerIllustration`,
    - `BoxIllustration`.
    - Update `KkbbFlashIllustration` color.
...
-  `Toggletip`: Add `bubbleColorText` props and update style.
...
- Icons: Add `megaphoneIconNext` and `BulbIconNext` icon components.
...
- `Autocomplete`: Update `autocomplete` prop.
...
- `RewardCard`: Add `hasShadow` prop, defaults to true.
...
- `EditorBar`: Add component.
- `TitleWithStroke`: Add `noMargin` prop.

Fix:

- `TextInput`: Fix `limit` initialization.
...
- `RewardCard`: Un-deprecate Info subcomponent.
...
- `DatePicker`: Remove component.
- `NavTabCarousel`: Remove component.
- `TextInput`: Update component.
- `TextCopy`: Update component.
- Simplifiy the following components:
  - `PasswordInput`
  - `TextInputWithButton`
  - `TextInputWithIcon`
  - `TextInputWithLimit`
...
- Tests: Remove tests not based on snapshots.

Features:

- `RichText`: Add component.
- `StandaloneRangeDatePicker`: Update component and dependencies.
- `SimpleCarousel`: Update component.
